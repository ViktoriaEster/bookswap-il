...
const BookList = ({ type }: BookListProps) => {
            ...
            case BOOK_LIST_TYPES.FAVORITES:
                return currentUser
                    ? books.filter((book: Book) =>
                        currentUser.favoriteBookIds.includes(book.id)
                    )
                    : [];
            default:
                return books;
    ...
    const activeBooks = useMemo(
        ...
        [books, activeFilter, type, currentUser]
    );
    ...
    return (
        <div>
            ...
            {activeBooks.length === 0 && (
                <div className={styles.notBooks}>Nothing here yet...</div>
            )}
            <div className={styles.bookListContainer}>
                    ...
                    <div className={styles.navMenuContainer}>
                        {navMenuItems.map((item: Author | City | Genre) => (
                            <div
                                ...
                                onClick={() => handleClickFilterBooks(item.id)}
                                key={item.id}
                            >
                                {item.name}
                            </div>
                ...
                <div className={styles.bookCardsContainer}>
                    {activeBooks.map((book: Book) => {
                        ...
                        const bookCity =
                            cities.find((city: City) => book.cityId === city.id) ?? null;
                        return (
                            <BookCardSmall
                                key={book.id}
                                book={book}
